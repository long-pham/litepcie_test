/*
 * Optimized LitePCIe DMA Test
 * 
 * This implementation improves on the original DMA test with:
 * - Asynchronous producer-consumer pattern with threads
 * - Memory access optimizations and prefetching
 * - Better performance monitoring
 * - Configurable buffer processing
 */
...
static pthread_t reader_thread, writer_thread;
...
static void* writer_thread_func(void *arg) {
    ...
    while (keep_running) {
        ...
        if (buf) {
            ...
            /* Process DMA immediately in loopback mode */
            if (!config.batch_process || dma_ctrl.loopback) {
                litepcie_dma_process(&dma_ctrl);
            }
        } else {
            /* No buffer available, process DMA */
            litepcie_dma_process(&dma_ctrl);
            usleep(1);
        }
    }
...
static void print_stats(void) {
    ...
    if (elapsed_s > 0) {
        ...
        printf("\r[%6.2fs] TX: %8.3f Gbps (%lu buffers) | RX: %8.3f Gbps (%lu buffers) | Errors: %lu",
...
static void usage(const char *prog) {
    ...
    printf("  -b             Disable batch processing\n");
...
int main(int argc, char *argv[]) {
    ...
    /* Parse options */
    while ((opt = getopt(argc, argv, "d:p:w:lznavbt:h")) != -1) {
        switch (opt) {
        case 'd':
            device = optarg;
            break;
    ...
    printf("Starting optimized DMA test...\n");
    ...
    printf("CPU affinity: %s, Batch processing: %s, Verification: %s\n",
           config.cpu_affinity ? "enabled" : "disabled",
           config.batch_process ? "enabled" : "disabled",
